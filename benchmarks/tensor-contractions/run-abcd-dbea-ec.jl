...
# TCCG benchmark 1: D_abcd = A_dbea * B_ec
...
@inline function Layout.store!(::Type{LayoutD{T}}, workspace, value, tile::Tile{size}) where {T, size}
    ...
    for i = 1 : NUMEL
        ...
        d = M % base.size(workspace, 4)
        b = (M ÷ base.size(workspace, 4)) % base.size(workspace, 2)
        a = (M ÷ base.size(workspace, 4)) ÷ base.size(workspace, 2)
